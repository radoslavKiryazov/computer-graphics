...
// Must match the radius used to build the ball mesh in JS.
...
fn main_vs(@location(0) inPos: vec2f) -> @builtin(position) vec4f {
    ...
    if (uniforms.scaleX > 1.0) {
        // Squashing: flatten more the closer a vertex sits to the bottom of
        // the ball. belowTop is 0 at the very top, 1 at the very bottom.
    ...
    let moved = vec2f(scaled.x + uniforms.ballX, scaled.y + uniforms.ballY);
