...
async function main() {
    ...
    // physics + spring-driven squash/stretch state machine
    function physicsStep() {
        if (state === 'jumping') {
            ...
            scaleY = 1 + springY;
            scaleX = 1 / scaleY;
            // Keep the ball's bottom edge pinned to the floor, same trick
            // as before: bottom = ballY - radius*scaleY.
            ballY = floorY + radius * scaleY;

            if (springVelocity > 0 && springY > -0.05) {
                // Settled enough: reset the spring and relaunch the ball.
                state = 'jumping';
                springY = 0;
                springVelocity = 0;
                scaleX = 1;
